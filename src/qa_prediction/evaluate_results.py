...
def eval_result(predict_file,method_type,alpha, cal_f1=True, topk = -1):
    # eval_name = "detailed_eval_result_top_{topk}.jsonl" if topk > 0 else 'detailed_eval_result_simi40_web.jsonl'
    # detailed_eval_file = predict_file.replace('predictions_simi40_web.jsonl', eval_name)
    eval_name = f"detailed_eval_result_top_{topk}.jsonl" if topk > 0 else f'detailed_eval_result_{method_type}_{alpha}.jsonl'
    detailed_eval_file = predict_file.replace(f'predictions_{method_type}_{alpha}.jsonl', eval_name)
    ...
    print(result_str)
    # result_name = "eval_result_top_{topk}.txt" if topk > 0 else 'eval_result_simi40_web.txt'
    # eval_result_path = predict_file.replace('predictions_simi40_web.jsonl', result_name)
    result_name = f"eval_result_top_{topk}.txt" if topk > 0 else f'eval_result_{method_type}_{alpha}.txt'
    eval_result_path = predict_file.replace(f'predictions_{method_type}_{alpha}.jsonl', result_name)
